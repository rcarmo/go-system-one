...
const GateUpGELUQ4KByWorkPTX = `.version 7.0
.target sm_80
.address_size 64

.visible .entry gate_up_gelu_q4_k_by_work(
    .param .u64 param_x,
    ...
    ld.param.u64 %rd3, [param_scales];
    ld.param.u64 %rd4, [param_mins];
    ...
    shl.b32 %r8, %r4, 1;     // outDim = 2*intermediate
    shr.u32 %r9, %r3, 8;     // nBlocks = inDim/256
    mul.lo.u32 %r10, %r1, %r3; // x batch base
    // base row for active expert = expert * outDim
    mul.lo.u32 %r11, %r7, %r8;
    add.u32 %r12, %r11, %r0; // gate row
    add.u32 %r13, %r12, %r4; // up row
...
loop_k:
    ...
    // gate row lookup: rowIndex = gateRow*nBlocks + block
