...
chunk=sys.argv[2]
diagnostic=os.environ.get('JEVBENCH_DIAGNOSTIC_CONTINUE_400')=='1'
assert not diagnostic or tier=='hard'
...
full_tasks=load_jsonl(str(root/'upstream'/'datasets'/'public'/(tier+'.jsonl')))
previous=[]
...
errors=0
for r in ordered:
    errors=errors+1 if not r['ok'] and r['status_code']!=422 and not(diagnostic and r['status_code']==400) else 0
...
manifest['diagnostic_continue_http400']=diagnostic
if diagnostic: manifest['protocol'] += ' DIAGNOSTIC ONLY: HTTP 400 input rejections retained as failed/wrong but do not trigger the consecutive-infrastructure-error stop; no retries. Original upstream-rule hard run stopped after 3 errors and is preserved separately.'
